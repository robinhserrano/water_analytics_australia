...
import 'package:water_analytics_australia/2_application/pages/aws_product_stocks_page/bloc/procuct_stock_cubit.dart';
...
class ProductStockPageWrapperProvider extends StatelessWidget {
  const ProductStockPageWrapperProvider({super.key});
...
class ProductStockPage extends StatefulWidget {
  ...
  static const name = 'productStock';
  static const path = '/productStock';
...
class _ProductStockPageState extends State<ProductStockPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: ProductStockPage._scaffoldKey,
      ...
      appBar: AppBar(
        ...
        actions: [
          // IconButton(
          //   onPressed: ProductStockPage.openDrawer,
          //   icon: const HeroIcon(
          //     HeroIcons.bars3,
          //     color: Colors.white,
          //   ),
      ...
      body: RefreshIndicator(
        ...
        onRefresh: () => context.read<ProductStockCubit>().fetchStocks(),
        child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: 4),
          child: BlocBuilder<ProductStockCubit, ProductStockCubitState>(
            builder: (context, state) {
              if (state is ProductStockStateLoading) {
              ...
              } else if (state is ProductStockStateLoaded) {
              ...
              } else if (state is ProductStockStateError) {
                return ProductStockError(
                  onRefresh: () =>
                      context.read<ProductStockCubit>().fetchStocks(),
                );
              }
...
class ProductStockLoaded extends StatefulWidget {
  ...
  @override
  State<ProductStockLoaded> createState() => _ProductStockLoadedState();
}

class _ProductStockLoadedState extends State<ProductStockLoaded> {
  ...
  @override
  void initState() {
    categories = widget.productStocks
        .map((e) => e.categName ?? '')
        .toList()
        .toSet()
        .toList()
  ...
  @override
  Widget build(BuildContext context) {
    final cubit = context.read<ProductStockCubit>();
    ...
    var filteredProducts = widget.productStocks
        .where(
          (e) =>
              e.warehouseId == selectedWarehouseId &&
              (selectedCategories.isEmpty ||
                  selectedCategories.contains(e.categName)),
        )
        .toList();
    ...
    filteredProducts.add(
      AwsProductStocks(
        ...
        displayName: '',
        categName: '',
        avgCost: filteredProducts.fold(
          0.0,
          (sum, product) => (sum ?? 0) + (product.avgCost ?? 0),
        ),
        totalValue: filteredProducts.fold(
          0.0,
          (sum, product) => (sum ?? 0) + (product.totalValue ?? 0),
        ),
        qtyAvailable: filteredProducts.fold(
          0.0,
          (sum, product) => (sum ?? 0) + (product.qtyAvailable ?? 0),
        ),
        freeQty: filteredProducts.fold(
          0.0,
          (sum, product) => (sum ?? 0) + (product.freeQty ?? 0),
        ),
        incomingQty: filteredProducts.fold(
          0.0,
          (sum, product) => (sum ?? 0) + (product.incomingQty ?? 0),
        ),
        outgoingQty: filteredProducts.fold(
          0.0,
          (sum, product) => (sum ?? 0) + (product.outgoingQty ?? 0),
        ),
        virtualAvailable: filteredProducts.fold(
          0.0,
          (sum, product) => (sum ?? 0) + (product.virtualAvailable ?? 0),
        ),
      ...
      body: filteredProducts.isEmpty
          ? Column(
              children: [
                ...
                const Expanded(
                  child: Center(
                    child: Text('No product stocks yet.'),
                  ),
                ),
              ],
            )
                      ...
                      GestureDetector(
                        child: Padding(
                          ...
                          child: Text(
                            'All Warehouses',
                            style: TextStyle(
                              fontWeight: selectedWarehouseId == 0
                                  ? FontWeight.bold
                                  : FontWeight.normal,
                            ),
                          ),
                        ...
                        (e) => GestureDetector(
                          child: Padding(
                            ...
                            child: Text(
                              e.name ?? '',
                              style: TextStyle(
                                fontWeight: selectedWarehouseId == e.warehouseId
                                    ? FontWeight.bold
                                    : FontWeight.normal,
                              ),
                            ),
                          ...
                          onTap: () => setState(() {
                            selectedWarehouseId = e.warehouseId;
                          }),
                        ),
                      ),
                      const SizedBox(
...
class ProductStockError extends StatelessWidget {
...
class MyDataTableSource extends DataTableSource {
  ...
  @override
  int get selectedRowCount => 0; // selectedSalesNo.length;
}
